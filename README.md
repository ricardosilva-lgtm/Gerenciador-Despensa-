...
## Caso o projeto seja retomado em semestres futuros ou expandido de forma independente, as seguintes melhorias são recomendadas:
Interface Gráfica (GUI): Migra uma interface de linha de comando (CLI) para uma aplicação visual (utilizando Lazarus/LCL).

Banco de Dados Relacional: Substituir o armazenamento em arquivo texto por um banco de dados relacional embarcado (como SQLite ou MySQL).
